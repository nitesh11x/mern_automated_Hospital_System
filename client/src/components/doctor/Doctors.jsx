...
import { Star, MessageCircle, Calendar, MapPin, ShieldCheck, User, CreditCard } from "lucide-react";
import { useDispatch, useSelector } from "react-redux";
import { getAllDoctorsThunk } from "../../redux/slices/doctor.slice";

/* ============================
    Doctor Registry Card (Memoized)
============================ */
        ...
        <div className="bg-white rounded-sm p-6 shadow-sm border  border-slate-200 hover:border-indigo-600 transition-all duration-300 group relative flex flex-col h-full">
            ...
            <div className="flex justify-between items-center ">
                ...
                <div className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-sm border border-slate-100">
                    ...
                    <span className="text-[11px] font-bold text-slate-900">{doc.rating || 4.5}</span>
                </div>
            </div>
                ...
                <div className="grid grid-cols-2 border-y border-slate-100 divide-x divide-slate-100">
                    <div className="py-3 pr-2">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">Experience</p>
                        </div>
                        <p className="text-xs font-bold text-slate-800 uppercase">{doc.experience} Years</p>
                    </div>
                    <div className="py-3 pl-4">
                        <div className="flex items-center gap-2 mb-1">
                            <CreditCard size={12} className="text-slate-400" />
                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">Consultation</p>
                        </div>
                        <p className="text-xs font-bold text-indigo-600 uppercase">${doc.consultationFees}</p>
                    </div>
                </div>

                {/* Action Buttons */}
                <div className="flex gap-2 pt-2">
                    <button className="flex-1 bg-indigo-600 text-white py-4 rounded-sm font-bold text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-md shadow-indigo-100">
            ...
            <div className="max-w-7xl mx-auto py-12">
                ...
                >
                    ...
                    <p className="text-slate-500 mt-4 text-sm font-medium max-w-2xl">
                        Browse our directory of verified healthcare professionals. All doctors are vetted for credentials and institutional compliance.
                    </p>
                </motion.header>
                ...
                {loading && (
                    <div className="flex flex-col items-center justify-center py-32 gap-4">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Loading Personnel Data...</p>
                    </div>
                )}
