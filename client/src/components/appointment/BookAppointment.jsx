...
const BookAppointment = () => {
  ...
  useEffect(() => {
    if (doctors && doctors.length === 0 && !doctorsLoading) {
      dispatch(getAllDoctorsThunk());
    }
  }, [dispatch, doctors, doctorsLoading]);
  ...
  const handleSelectPreviousAppointment = (appointment) => {
    ...
    if (doctor) {
      ...
      setFormData((prev) => ({
        ...prev,
        doctorId: doctor._id,
        selectedDocName: `Dr. ${doctor.firstName} ${doctor.lastName} (${doctor.specialization})`,
                  ...
                  <SummaryItem
                    ...
                    value={
                      formData.appointmentDate
                        ? `${new Date(
                            formData.appointmentDate
                          ).toLocaleDateString()} @ ${selectedSlot?.time || "TBD"}`
                        : "NOT SCHEDULED"
                    }
                  />
                  <SummaryItem
