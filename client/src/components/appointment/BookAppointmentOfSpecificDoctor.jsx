...
const BookAppointmentOfSpecificDoctor = () => {
  ...
  const selectedDoctor = useMemo(() => {
    ...
    return doctors.find((doc) => doc._id?.toString() === doctorId?.toString()) || null;
  }, [doctorId, doctors]);
  ...
  useEffect(() => {
    if (selectedDoctor) {
      setFormData((prev) => ({
        ...
        selectedDocName: `Dr. ${selectedDoctor.firstName} ${selectedDoctor.lastName} (${selectedDoctor.specialization})`,
      }));
    } else if (doctorId) {
      setFormData((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    if (formData.isVisited && formData.email) {
      fetchPreviousAppointmentsByEmail(formData.email);
    }
  }, [formData.email, formData.isVisited, fetchPreviousAppointmentsByEmail]);
  ...
  useEffect(() => {
    if (bookingSuccess) {
      ...
      setFormData({
        doctorId: doctorId || "",
        selectedDocName: selectedDoctor
  ...
  }, [bookingSuccess, error, dispatch, doctorId, patientFromId, selectedDoctor, formData.email, formData.name]);
  ...
  const handleSubmit = (e) => {
    ...
    const submissionData = {
      ...
      previousAppointmentId:
        selectedPreviousAppointment?._id || formData.previousAppointmentId || null,
    };
            ...
            <section className="bg-white border border-slate-200 rounded-sm shadow-sm p-8">
              ...
              <div className="relative z-50">
                <button
                  ...
                  className="w-full flex items-center justify-between px-6 py-4 bg-slate-50 border border-slate-200 rounded-sm text-sm font-bold transition-all hover:bg-white focus:border-indigo-600"
                  disabled={!!doctorId}
                >
                  ...
                  {!doctorId && (
                    <ChevronDown
                      size={18}
                      className={`text-indigo-600 transition-transform ${isOpen ? "rotate-180" : ""}`}
                    />
                  )}
                </button>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setFormData((prev) => ({
                                    ...prev,
                                    doctorId: doc._id,
                                    selectedDocName: `Dr. ${doc.firstName} ${doc.lastName} (${doc.specialization})`,
                                  }));
                                  setIsOpen(false);
                                }}
                              ...
                              >
                                <div>
                                  ...
                                  <p className="text-[8px] font-bold opacity-40 mt-1">
                                    ₹{doc.consultationFees} | {doc.experience}+ yrs
                                  </p>
                                </div>
                        ...
                        <div className="space-y-4">
                          {previousAppointments.map((appointment) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                handleSelectPreviousAppointment(appointment)
                              }
            ...
            <div className="bg-slate-900 rounded-lg p-8 sticky top-28 border border-slate-800 shadow-2xl">
              <div className="relative z-10">
                ...
                <div className="space-y-6">
                  ...
                  <SummaryItem
                    label="Patient"
                    value={formData.name || "UNREGISTERED"}
                  />
