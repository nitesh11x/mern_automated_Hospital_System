...
const Stories = () => {
    ...
    }, [current, currentReview?.video, isPlaying, hasUserInteracted]);
    ...
    const handlePlayPause = async () => {
        if (!videoRef.current || !currentReview?.video) {
    ...
    const handleFileSelect = (e) => {
        ...
        setError("Please upload a supported image or video file");
    };
    ...
    const statCards = [
        { icon: Users, value: `${transformedReviews.length}+`, label: "Patient Stories", iconClass: "text-indigo-600" },
        { icon: Award, value: `${Math.round((transformedReviews.reduce((sum, r) => sum + r.rating, 0) / transformedReviews.length / 5) * 100)}%`, label: "Satisfaction Rate", iconClass: "text-purple-600" },
        { icon: Heart, value: `${Math.floor(transformedReviews.length * 0.8)}+`, label: "Success Stories", iconClass: "text-pink-600" },
        { icon: ShieldCheck, value: "24/7", label: "Patient Support", iconClass: "text-emerald-600" },
    ];
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {selectedDoctorId && Array.isArray(doctors) && (
                                            <p className="text-xs text-indigo-600">
                                                You're reviewing: Dr.{" "}
                                                {doctors.find((d) => d._id === selectedDoctorId)?.firstName}{" "}
                                            ...
                                            <div className="relative rounded-sm overflow-hidden bg-slate-100">
                                                {mediaType === "image" ? (
                                                    ...
                                                    <video
                                                        ...
                                                        playsInline
                                                        onError={() => {
                                                            setError("Video preview failed to load. Please upload a supported MP4 or WebM file.");
                                                            setMediaPreview(null);
                                                            setMedia(null);
                                                            setMediaType(null);
                                                        }}
                                        ...
                                        <div className="flex items-center gap-4 pt-4 border-t border-indigo-100">
                                            <div className="relative">
                                                <img
                                                    ...
                                                    onError={(e) => {
                                                        e.currentTarget.src =
                                                            "https://ui-avatars.com/api/?name=Patient&background=6366f1&color=fff&bold=true";
                                                    }}
                                                />
                                    ...
                                    <div className="relative">
                                        {currentReview.video && !videoLoadFailed ? (
                                            <div className="relative group/video rounded-sm overflow-hidden bg-slate-900 shadow-xl">
                                                ...
                                                {!isPlaying && isVideoReady && !videoLoadFailed && (
                                                    <button
                                            ...
                                            <div className="relative rounded-sm overflow-hidden bg-linear-to-br from-indigo-50 to-purple-50 p-8 text-center">
                                                <div className="absolute inset-0 opacity-10">
                                                    <Heart
                                                        size={120}
                                                        className="text-indigo-600 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
                                                    />
                                                </div>
