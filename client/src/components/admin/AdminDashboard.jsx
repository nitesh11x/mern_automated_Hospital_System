...
const AdminDashboard = () => {
    ...
    // --- REDUX STATE ---
    const { isAdminAuthenticated, admin, stats } = useSelector((state) => state.admin);
        ...
        <div className="flex min-h-screen bg-[#F8FAFC] font-sans">
            ...
            <aside className="w-64 bg-slate-900 text-slate-400 hidden lg:flex flex-col p-4 pt-16 sticky top-0 h-screen">
                ...
                <nav className="flex-1 space-y-1">
                    <SidebarBtn icon={<LayoutDashboard size={18} />} label="Operational Hub" active={activeTab === "dashboard"} onClick={() => setActiveTab("dashboard")} />
                    <SidebarBtn icon={<Stethoscope size={18} />} label="Medical Faculty" active={activeTab === "doctors"} onClick={() => setActiveTab("doctors")} />
                    <SidebarBtn icon={<Users size={18} />} label="Patient Registry" active={activeTab === "patients"} onClick={() => setActiveTab("patients")} />
                    <SidebarBtn icon={<Calendar size={18} />} label="Appointments" active={activeTab === "appointments"} onClick={() => setActiveTab("appointments")} />
                </nav>
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-slate-900 pt-10 uppercase tracking-tighter">
                                {activeTab === "dashboard" ? "Command Dashboard" :
                                    activeTab === "doctors" ? "Medical Faculty" :
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-10">
                                ...
                                <ActionCard title="Patient Operations" icon={<Users />}>
                                    <div className="grid grid-cols-2 gap-2 mt-4">
                                        ...
                                        <GridBtn icon={<ClipboardList />} label="Manage All" onClick={() => setActiveTab("patients")} color="bg-slate-700" />
                                        <GridBtn icon={<UserCheck />} label="Triage Status" onClick={() => setActiveTab("appointments")} color="bg-slate-700" />
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="flex items-center justify-between mb-8">
                                        ...
                                        <div className="flex gap-4">
                                            <div className="text-right">
                                                <p className="text-lg font-black text-slate-900 leading-none">{stats?.totalPatients || 0}</p>
                                                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Active Admissions</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-lg font-black text-slate-900 leading-none">{stats?.totalDoctors || 0}</p>
                                                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Medical Faculty</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-lg font-black text-slate-900 leading-none">{stats?.totalAppointments || 0}</p>
                                                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Total Schedules</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-lg font-black text-slate-900 leading-none">₹{stats?.totalRevenue || 0}</p>
                                ...
                                <div className="bg-white border border-slate-200 rounded p-6 shadow-sm">
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        <button onClick={() => setActiveTab("doctors")} className="text-[9px] font-black text-indigo-600 uppercase hover:underline">View All</button>
                                    </div>
...
const GridBtn = ({ icon, label, to, onClick, color }) => {
    if (onClick) {
        return (
            <button onClick={onClick} className={`${color} p-3 rounded text-white flex flex-col items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-md shadow-slate-200 w-full`}>
    ...
    return (
        <a href={to} className={`${color} p-3 rounded text-white flex flex-col items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-md shadow-slate-200 w-full`}>
            {React.cloneElement(icon, { size: 18, strokeWidth: 2.5 })}
            <span className="text-[9px] font-black uppercase tracking-tighter text-center leading-tight">{label}</span>
        </a>
    );
...
const SidebarBtn = ({ icon, label, active = false, onClick }) => (
    <button onClick={onClick} className={`w-full flex items-center gap-4 px-4 py-3 rounded transition-all text-xs font-bold uppercase tracking-widest ${active ? "bg-indigo-600 text-white shadow-lg shadow-indigo-900/20" : "hover:bg-slate-800 hover:text-white"
        }`}>
        {icon}
